...
const ShopSlot = ({ index, item, onOpen }: { index: string, item?: ShopItemData, onOpen: (item: ShopItemData) => void }) => {
  if (item) {
    return (
      <Box 
        border="1px solid"
      ...
      >
        ...
        <Center h="full" flexDirection="column">
          ...
          <Box mt={2} bg="white" px={2} py={0.5}>
            <Text color="black" fontSize="10px" fontWeight="900" fontFamily="monospace">{item.priceString}</Text>
          </Box>
        </Center>
      </Box>
    )
  ...
  return (
    <Box bg="black" minH="100vh" pb="100px">
      <Container maxW="container.sm" p={0}>
        ...
        <Box p={6} borderBottom="1px solid whiteAlpha.300">
          <Flex justify="space-between" align="center">
            <HStack spacing={4}>
              <Logo boxSize="35px" color="white" />
              <VStack align="start" spacing={0}>
                <Text fontSize="8px" fontWeight="900" color="#FFB000" fontFamily="monospace">PAPILLON</Text>
                <Heading fontSize="3xl" fontWeight="900" fontStyle="italic" color="white" lineHeight="1" fontFamily="'Archivo Black', sans-serif">
            ...
            >
              <Icon as={PiShoppingBagFill} color="white" boxSize="20px" />
              {cart.length > 0 && (
                <Circle 
                  size="14px" 
                  bg="#FFB000" 
        ...
        <Box borderY="1px solid whiteAlpha.300" px={6} py={2}>
          <Flex justify="space-between" align="center">
            <Text fontSize="7px" fontWeight="900" color="white" fontFamily="monospace">PROTOCOL: {mode === 'physical' ? 'PHYSICAL' : 'DIGITAL'}_MARKET</Text>
            <Text fontSize="7px" fontWeight="900" color="white" fontFamily="monospace">ENCRYPTION: WNGS_SYNC</Text>
          </Flex>
        </Box>
        ...
        <Box p={6}>
          <Box border="1px solid white" borderBottom="none" p={4} bg="black">
            <Flex justify="space-between" align="center">
              ...
              <Menu>
                ...
                <MenuList bg="black" border="1px solid white" borderRadius="0" minW="100px">
                  <MenuItem bg="black" color="white" fontSize="9px" fontWeight="900" fontFamily="monospace" _hover={{ bg: "whiteAlpha.200" }} onClick={() => setFilter('ALL')}>ALL_ASSETS</MenuItem>
                  <MenuItem bg="black" color="white" fontSize="9px" fontWeight="900" fontFamily="monospace" _hover={{ bg: "whiteAlpha.200" }} onClick={() => setFilter('PREMIUM')}>PREMIUM_PROTOCOL</MenuItem>
                  <MenuItem bg="black" color="white" fontSize="9px" fontWeight="900" fontFamily="monospace" _hover={{ bg: "whiteAlpha.200" }} onClick={() => setFilter('BASIC')}>BASIC_PROTOCOL</MenuItem>
                </MenuList>
              </Menu>
            </Flex>
          </Box>
          ...
          <Box border="1px solid white" p={4}>
            <Flex justify="space-between" mb={4} borderBottom="1px solid whiteAlpha.300" pb={1}>
              <Text fontSize="6px" fontWeight="900" color="whiteAlpha.600" fontFamily="monospace">ID_INDEX</Text>
              <Text fontSize="6px" fontWeight="900" color="whiteAlpha.600" fontFamily="monospace">VALUATION</Text>
            </Flex>
            
            <SimpleGrid columns={3} spacing={3}>
              {/* Render Filtered Items */}
              {filteredItems.map((item, idx) => (
                <ShopSlot key={item.id} index={(idx + 1).toString().padStart(2, '0')} item={item} onOpen={handleItemOpen} />
              ))}
              
              {/* Fill remaining slots with empty dashed boxes (up to 9) */}
              {Array.from({ length: Math.max(0, 9 - filteredItems.length) }).map((_, idx) => (
                <ShopSlot key={`empty-${idx}`} index={(filteredItems.length + idx + 1).toString().padStart(2, '0')} onOpen={handleItemOpen} />
              ))}
            </SimpleGrid>
            ...
            {selectedItem && (
              <Box perspective="1000px" w="full" h="500px">
                ...
                >
                  ...
                  >
                    <VStack spacing={8}>
                      <Box border="2px solid white" p={6}>
                  ...
                  >
                    <VStack spacing={6} w="full">
                      <VStack spacing={1}>
                        <Text fontSize="xs" fontWeight="900" color="black">ASSET_SPECIFICATIONS</Text>
                      ...
                      <VStack align="start" spacing={4} w="full">
                        {selectedItem.type === 'physical' && (
                          <Box w="full">
                            <Text fontSize="7px" fontWeight="900" color="blackAlpha.600" mb={2} fontFamily="monospace">SELECT_SIZE</Text>
                            <HStack spacing={3}>
                              {['S', 'M', 'L', 'XL'].map((size) => (
                                <Center
                                  key={size}
                                  ...
                                  borderRadius="full"
                                  border="1px solid"
                                  borderColor={selectedSize === size ? "black" : "blackAlpha.300"}
                                  bg={selectedSize === size ? "black" : "transparent"}
                        ...
                        {selectedItem.specs.map((spec, i) => (
                          <Box key={i}>
                            <Text fontSize="8px" fontWeight="900" color="blackAlpha.600">{spec.label}</Text>
                            <Text fontSize="xs" fontWeight="700" color="black">{spec.value}</Text>
                          </Box>
                        ))}
                      </VStack>
      ...
      <Modal isOpen={isCartOpen} onClose={onCartClose} isCentered size="sm">
        <ModalOverlay backdropFilter="blur(10px)" bg="blackAlpha.900" />
        <ModalContent bg="black" border="2px solid white" borderRadius="20px" overflow="hidden">
          <ModalBody p={0}>
            <VStack spacing={0} align="stretch">
              ...
              <Box p={6} borderBottom="1px solid whiteAlpha.300">
                <Flex justify="space-between" align="center">
                  <HStack spacing={3}>
                    ...
                    <Heading fontSize="xl" fontWeight="900" color="white" fontStyle="italic">
                      MY_CART
                    </Heading>
              ...
              <Box minH="300px" p={6} overflowY="auto" maxH="40vh">
                {cart.length === 0 ? (
                  <Center h="250px" flexDirection="column">
                    <Icon as={PiShoppingBagFill} color="whiteAlpha.200" boxSize="60px" mb={4} />
                    <Text fontSize="10px" fontWeight="900" color="whiteAlpha.400" fontFamily="monospace">
                      CART_IS_EMPTY
                    </Text>
                  </Center>
                ) : (
                  <VStack spacing={4} align="stretch">
                    {cart.map((item, idx) => (
                      <Flex key={idx} justify="space-between" align="center" borderBottom="1px solid whiteAlpha.100" pb={2}>
                        <HStack spacing={4}>
                          <Box border="1px solid white" p={2}>
                            <TShirtIcon boxSize="20px" />
                          </Box>
                          <VStack align="start" spacing={0}>
                            <Text fontSize="10px" fontWeight="900" color="white">{item.name}</Text>
                            <Text fontSize="8px" fontWeight="900" color="whiteAlpha.400" fontFamily="monospace">${item.price}</Text>
                          </VStack>
                        </HStack>
                        <IconButton 
                          aria-label="Remove" 
              ...
              {/* Cart Footer */}
              <Box p={6} bg="black" borderTop="1px solid whiteAlpha.300">
                <VStack spacing={4} align="stretch">
                  <Flex justify="space-between" align="center">
                    <Text fontSize="8px" fontWeight="900" color="whiteAlpha.600" fontFamily="monospace">ESTIMATED_REWARDS</Text>
                  ...
                  <Flex justify="space-between" align="center">
                    <Text fontSize="8px" fontWeight="900" color="whiteAlpha.600" fontFamily="monospace">SUBTOTAL</Text>
                    <Text fontSize="2xl" fontWeight="900" color="white" fontStyle="italic">${subtotal}</Text>
                  </Flex>
